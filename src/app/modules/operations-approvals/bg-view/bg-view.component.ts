...
@Component({
    ...
    templateUrl: './bg-view.component.html',
    styleUrls: ['./bg-view.component.scss'],
})
export class BgViewComponent {
    ...
    constructor(
        ...
        public dialogRef: MatDialogRef<BgViewComponent>,
    ...
    ngOnInit(): void {
        ...
        this.parentData.isAmmended =
            this.parentData?.isAmmended === 'Y'
                ? '' + this.pageType + ' Amendment Request'
                : 'New ' + this.pageType + ' Request';
        this.fileDocuments = this.parentData.documents ? this.parentData.documents : [];
        let requestObject: any = {};
        const serviceRequestdata = this.parentData;
        requestObject = { ...serviceRequestdata };
        this.transactions.push(requestObject);
        this.showHistory = false;
        if (this.data.type !== 'historyAction') {
            this.getHistory(this.applnId);
        }
    }
    ...
    public getHistory(applnId: any) {
        let refNo = this.parentData.parentBg ? this.parentData.parentBg : this.parentData.id;
        ...
        if (file) {
            ...
            if (this.fileErrMsg === '') {
                ...
                this.sandBox.fileUploadHisotry(this.pageType, payload, formData).subscribe((res: any) => {
                    ...
                    this.fileDocuments = res.data.documents ? res.data.documents : [];
                });
            }
        }
    }
